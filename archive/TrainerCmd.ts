...
const TrainerCmd = {
    ...
    data: new SlashCommandBuilder()
        ...
        .addSubcommand((subCommand) =>
            subCommand.setName('profile').setDescription('Setup your trainer profile')
        )
        .addSubcommand((subCommand) => subCommand.setName('team').setDescription('Set your team'))
        .addSubcommand((subCommand) =>
            subCommand.setName('delete').setDescription('Delete your trainer profile')
        )
        .addSubcommand((subCommand) =>
            subCommand
                .setName('code')
    ...
    async execute(interaction: ChatInputCommandInteraction) {
        ...
        switch (subCommand) {
            case 'profile':
                this.executeProfile(interaction);
                break;
            case 'team':
                this.executeTeam(interaction);
                break;
            case 'delete':
                this.executeDelete(interaction);
                break;
            case 'code':
                this.executeShowOrCode(interaction, subCommand);
                break;
            case 'show':
                this.executeShowOrCode(interaction, subCommand);
                break;
            default:
                await interaction.reply({
                    content: `Trainer profile management not yet implemented -- ${subCommand}`,
    ...
    async autocomplete(interaction: AutocompleteInteraction) {
        ...
        switch (subCommand) {
            case 'code':
                this.autocompleteShowOrCode(interaction, subCommand);
                break;
            case 'show':
                this.autocompleteShowOrCode(interaction, subCommand);
                break;
            default:
                client.logger.error(
                    `Trainer profile command autocomplete not yet implemented for subcommand -- ${subCommand}`
                );
        }
    },
    ...
    async executeDelete(interaction: ChatInputCommandInteraction) {
        await interaction.reply({
            content: `Delete -- Trainer profile management not yet implemented`,
            flags: MessageFlags.Ephemeral,
        });
    },
    ...
    async autocompleteShowOrCode(interaction: AutocompleteInteraction, subCommand: string) {
        ...
        client.logger.debug(
            `Initiating autocomplete for ${subCommand} -- ${this.data.name} :: ${focusedOption.name} :: ${focusedOption.value}`
        );
