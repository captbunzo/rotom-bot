...
const PogoHubLinkCmd = {
    ...
    async autocomplete(interaction) {
        ...
        switch (subCommand) {
            ...
            default :
                client.logger.error(`Pogo Hub Link management command autocomplete not yet implemented for subcommand -- ${subCommand}`);
        }
    },
    ...
    async executeLoad(interaction) {
        ...
        for (const pogoHubLinkLine of pogoHubLinkLines) {
            ...
            let masterPokemon = await MasterPokemon.get({ pokedexId: pokedexId, form: form, unique: true });
            ...
            if (masterPokemon === null) {
                ...
                let wikiLink = await PogoHubLink.get({ id: pogoHubLinkObj.id, unique: true });
