...
const WikiLinkCmd = {
    ...
    async autocomplete(interaction) {
        ...
        switch (subCommand) {
            ...
            default :
                client.logger.error(`Wiki Link management command autocomplete not yet implemented for subcommand -- ${subCommand}`);
        }
    },
    ...
    async executeLoad(interaction) {
        ...
        for (const wikiLinkLine of wikiLinkLines) {
            ...
            let masterPokemon = await MasterPokemon.get({ pokemonId: pokemon.toUpperCase(), form: form, unique: true });

            // See if we can find it by removing dashes
            if (!masterPokemon) {
                const pokemonAltCheck = pokemon.replace('-', '');
                masterPokemon = await MasterPokemon.get({ pokemonId: pokemonAltCheck.toUpperCase(), form: form, unique: true });
            ...
            if (masterPokemon === null) {
                ...
                let wikiLink = await WikiLink.get({ id: wikiLinkObj.id, unique: true });
