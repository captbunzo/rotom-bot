...
import PokedexRegister from '@/components/compound/PokedexRegisteryComponent.js';
...
const PokedexCmd = {
        ...
        .addSubcommand((subCommand) =>
            subCommand
                ...
                .addIntegerOption((option) =>
                    option.setName('pokédex-id').setDescription('Pokédex ID').setRequired(false)
                )
        ),
    /*
        ...
        switch (subCommand) {
            case 'register':
                this.executeRegister(interaction);
                break;
            case 'need':
                this.executeNeed(interaction);
                break;
            default:
                await interaction.reply({
                    content: `Pokédex management not yet implemented -- ${subCommand}`,
    ...
    async autocomplete(interaction: AutocompleteInteraction) {
        ...
        switch (subCommand) {
            case 'register':
                this.autocompleteRegister(interaction);
                break;
            case 'need':
                this.autocompleteNeed(interaction);
                break;
            default:
                client.logger.error(
                    `Pokédex command autocomplete not yet implemented for subcommand -- ${subCommand}`
                );
        }
    },
    ...
    async executeNeed(interaction: ChatInputCommandInteraction) {
        await interaction.reply({
            content: 'Pokédex need subcommand not yet implemented',
            flags: MessageFlags.Ephemeral,
        });
    },
