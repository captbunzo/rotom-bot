...
export class MasterCPM extends DrossDatabaseTable {
    ...
    get cpm   (): number { return this.getField('cpm'); }
    ...
    static async getCombatPower(masterPokemon: MasterPokemon, attackIV: number, defenseIV: number, staminaIV: number, level: number) {
        ...
        if (!masterPokemon.baseAttack) {
            throw new Error(`MasterPokemon with templateId ${masterPokemon.templateId} does not have a baseAttack value`);
        }

        if (!masterPokemon.baseDefense) {
            throw new Error(`MasterPokemon with templateId ${masterPokemon.templateId} does not have a baseDefense value`);
        }

        if (!masterPokemon.baseStamina) {
            throw new Error(`MasterPokemon with templateId ${masterPokemon.templateId} does not have a baseStamina value`);
        }
