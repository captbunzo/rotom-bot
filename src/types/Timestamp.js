...
export default class Timestamp {
    ...
    constructor(timezone, dateValue) {
        ...
        const offsetDate  = new Date(serverDate.toLocaleString(this.locales, {timeZone: timezone}));
        const offsetHours = Math.round((serverDate.getTime() - offsetDate.getTime()) / (1000 * 60)) / 60;
    ...
    static timeIsValid(timeString) {
        return timeString.match(Timestamp.TIME_12H_PATTERN)
    ...
    getMillisec = (timezone) => parseInt(this.format24hour({fractionalSecondDigits: 3}, timezone));
